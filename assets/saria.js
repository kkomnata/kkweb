...
{	
	...
	try
	{
		sariaSocket = new WebSocket((sariaSettings.secure ? 'wss' : 'ws')+'://'+sariaSettings.server+':'+sariaSettings.port+'/'+sariaSettings.additionalPath);
		...
		case 'AUTH' :
			switch (commands[1])
			{
				...
				case 'READONLY':
					if (sariaSettings.readonly)
					{
						$('#chat-message-field').hide();
						$('#chat-settings-button').hide();
						$('#chat-unauthorized').show();
					}
					else
					{
						$('#chat-message-field').show();
						$('#chat-settings-button').show();
						$('#chat-unauthorized').hide();
		...
		case 'LOGCON'  :
			...
			/*
				<div class="chat-message">
					<span class="chat-message-time">13:37</span>
					<span class="chat-message-id">14880</span>
					<span class="chat-message-name">Айниняя</span>
					<span class="chat-message-content">Как это сусно, зига!</span>
				</div>
