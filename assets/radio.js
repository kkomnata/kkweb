...
function radioInit()
{
	try
	{
		...
		// * In case of AAC support by server uncomment this
		/* 
			...
			if (a.canPlayType('audio/aac') != 'no' &&
				...
				currentChannel = 'kkomnata';
			else
				currentChannel = 'kkomnata.mp3';
...
function radioPlay(channel)
{
	...
	if (playerReady)
	{
		...
		radioPlayer.src = '//'+BASE_DOMAIN+'/station/'+channel+'?'+randword();
...
function setTempTitle(tx)
{
	// this will used if we'll in need of showing current track info
}